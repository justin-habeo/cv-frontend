export const getHybridChartOptions = (data, chartConfig, muiTheme) => {
  console.log("Data received:", data);
  console.log("Chart config received:", chartConfig);

  if (!Array.isArray(data) || data.length === 0) {
    console.error("Invalid data format for hybrid chart:", data);
    return {};
  }
  ...
  const series = data.map((seriesData, index) => {
    ...
    const seriesType = seriesConfig.seriesType || 
                       (seriesData.name.includes('sum') || seriesData.name.includes('total') ? 'bar' : 'line');
    return {
      name: seriesData.name || `Series ${index + 1}`,
      type: seriesType,
      yAxisIndex: seriesType === 'bar' ? 1 : 0,
