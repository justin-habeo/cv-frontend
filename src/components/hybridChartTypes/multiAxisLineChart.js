// src/components/hybridChartTypes/multiAxisLineChart.js

export const getMultiAxisLineChartOptions = (data, chartConfig, muiTheme) => {
  console.log("Data received:", data);
  console.log("Chart config received:", chartConfig);

  if (!Array.isArray(data) || data.length === 0) {
    console.error("Invalid data format for multi-axis line chart:", data);
    return {};
  }
  ...
  const yAxis = series.map((s, index) => ({
    ...
    axisLine: { 
      show: true,
      lineStyle: { color: muiTheme.palette.text.secondary } 
    },
