// src/components/hybridChartTypes/lineWithThresholdChart.js

export const getLineWithThresholdChartOptions = (data, chartConfig, muiTheme) => {
  console.log("Data received:", data);
  console.log("Chart config received:", chartConfig);

  if (!Array.isArray(data) || data.length === 0) {
    console.error("Invalid data format for line with threshold chart:", data);
    return {};
  }
  ...
  const thresholds = chartConfig.additionalConfig?.thresholds || [];
  ...
  const chartOptions = {
    ...
    legend: {
      data: [...series.map(s => s.name), ...thresholds.map(t => t.name)],
      bottom: 0,
      textStyle: { color: muiTheme.palette.text.primary },
    },
    grid: {
    ...
    series: [
      ...
      ...thresholds.map(threshold => ({
        ...
        markLine: {
          silent: true,
          lineStyle: {
            color: threshold.color || muiTheme.palette.warning.main,
            type: threshold.lineType || 'solid',
            width: threshold.lineWidth || 2,
          },
          data: [{
            yAxis: threshold.value,
            label: {
              formatter: `{c} (${threshold.name})`,
            },
          }]
        },
      }))
